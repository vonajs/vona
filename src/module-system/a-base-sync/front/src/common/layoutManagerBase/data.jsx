export default {
  ...
  methods: {
    async data_adapterInit() {
      ...
      this.data.adapter = await this.$meta.util.createComponentInstanceByName(configComponent, options);
    },
    async data_providerInit(optionsDefault) {
      // config options is preffered
      const configOptions = this.layout.config.options?.dataProvider;
      const options = this.$meta.util.extend({}, optionsDefault, configOptions);
      return await this.data_providerSwitch(options);
    },
