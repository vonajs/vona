...
export class BeanAtomDraft extends BeanAtomClone {
  async closeDraft({ key: keyOuter, atomClass: atomClassOuter, options: optionsOuter }: any) {
    ...
    if (atomDraft.atomIdFormal) {
      ...
      await this.modelAtom.update({
        id: atomIdDraft,
        atomClosed: 1,
        atomFlowId: atomDraft.atomFlowId, // hold atomFlowId
      });
    } else {
  ...
  async openDraft({ key: keyOuter, atomClass: atomClassOuter, options: optionsOuter, user }: any) {
    ...
    // check simple switch
    atom = await (this as unknown as BeanAtomSimple)._checkSimpleSwitch({ atomClass, atomClassBase, atom, user });
    // open draft
    let res;
    if (atom.atomSimple) {
      // simple
  ...
  async _openDraft_asSimple({ atomClass, atom, user }: any) {
    ...
    if (atom.atomStage === 2) {
      ...
      keyFormal = await (this as unknown as BeanAtomClone)._copy({
        target: 'formal',
        atomClass,
  ...
  async _openDraft_asSimpleZero_history({ atomClass, atom, user }: any) {
    ...
    if (changed) {
      ...
      keyDraft = await (this as unknown as BeanAtomClone)._copy({
        target: 'draft',
        atomClass,
        srcKey: { atomId: atom.id },
        srcItem: null,
