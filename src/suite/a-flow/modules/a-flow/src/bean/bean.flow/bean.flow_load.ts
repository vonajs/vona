...
export class BeanFlowLoad extends BeanFlowAssignees {
  async _loadFlowInstance({ flowId, history, throwError = true }: any) {
    ...
    if (!flow) {
      if (throwError) {
        this.self.scope.error.FlowNotFound__.throw(flowId);
      } else {
        return null;
      }
    }
    ...
    if (!flowDef) this.self.scope.error.FlowDefinitionNotFound__.throw(flow.flowDefId);
  ...
  async _loadFlowNodeInstance({ flowNodeId, history, throwError = true }: any) {
    ...
    if (!flowNode) {
      if (throwError) {
        this.self.scope.error.FlowNodeNotFound__.throw(flowNodeId);
      } else {
        return null;
      }
    }
