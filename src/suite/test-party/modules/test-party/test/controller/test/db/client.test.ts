...
describe.only('test/controller/test/db/client.test.js', () => {
  it('action:client', async () => {
    ...
    // client
    const clientDefault = ctx.bean.scope('a-db').local.client;
    const clientDefault2 = ctx.bean.scope('a-db').local.client.get();
    const clientDefault3 = ctx.bean.scope('a-db').local.client.get();
