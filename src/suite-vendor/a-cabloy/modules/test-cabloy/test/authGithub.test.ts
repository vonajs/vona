      ...
      async () => {
        ...
        const [isAuthenticated2, _err] = await catchError(async () => {
          return await app.bean.executor.newCtxIsolate(async () => {
            return await app.bean.executor.performAction('get', '/test/cabloy/passport/isAuthenticated');
          });
        });
        ...
        const isAuthenticated = await app.bean.executor.newCtxIsolate(async () => {
          return await app.bean.executor.performAction('get', '/test/cabloy/passport/isAuthenticated', { authToken: jwt?.accessToken });
        });
        assert.equal(isAuthenticated, true);
        // current
        const current: IPassport = await app.bean.executor.newCtxIsolate(async () => {
          return await app.bean.executor.performAction('get', '/test/cabloy/passport/current', { authToken: jwt?.accessToken });
        });
