...
describe('ssr.test.ts', () => {
  it('action:ssr:retrieveMenus:en-us', async () => {
    await app.bean.executor.mockCtx(async () => {
      ...
      // publicPath: second
      menus = await app.bean.menu.retrieveMenus('second');
      menu = menus.menus?.find(item => item.title === 'Home');
      assert.equal(!!menu, false);
      group = menus.groups?.find(item => item.title === 'Tools');
  ...
  it('action:ssr:retrieveMenus:zh-cn', async () => {
    await app.bean.executor.mockCtx(async () => {
      ...
      // publicPath: second
      menus = await app.bean.menu.retrieveMenus('second');
      menu = menus.menus?.find(item => item.title === '首页');
      assert.equal(!!menu, false);
      group = menus.groups?.find(item => item.title === '工具');
      assert.equal(!!group, true);
    }, { locale: 'zh-cn' });
  });
});
