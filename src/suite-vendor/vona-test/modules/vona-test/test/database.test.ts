...
describe('database.test.ts', () => {
  it('action:dataSource:switch', async () => {
    await app.bean.executor.mockCtx(async () => {
      // current
      assert.equal(app.ctx.dbMeta.currentClient.clientName, app.config.database.defaultClient);
      // switch
      ...
      await app.bean.dataSource.switch(async () => {
        assert.equal(app.ctx.dbMeta.currentClient.clientName, clientName2);
