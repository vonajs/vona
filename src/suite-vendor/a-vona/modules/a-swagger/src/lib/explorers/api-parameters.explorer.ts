...
import { ParameterMetadataAccessor, ParamWithTypeMetadata } from '../services/parameter-metadata-accessor.js';
...
const parametersMetadataMapper = new ParametersMetadataMapper(modelPropertiesAccessor);
const swaggerTypesMapper = new SwaggerTypesMapper();
const schemaObjectFactory = new SchemaObjectFactory(modelPropertiesAccessor, swaggerTypesMapper);
...
export const exploreApiParametersMetadata = (
  ...
  const explicitParameters: any[] = Reflect.getMetadata(DECORATORS.API_PARAMETERS, method);
  ...
  const noExplicitAndGlobalMetadata = isNil(explicitParameters) && isNil(globalParameters);
  ...
  if (!noExplicitAndGlobalMetadata) {
    ...
    properties = unionWith(properties, explicitParameters, globalParameters, (arrVal, othVal) => {
...
function removeBodyMetadataIfExplicitExists(properties: ParamWithTypeMetadata[], explicitParams: any[]) {
