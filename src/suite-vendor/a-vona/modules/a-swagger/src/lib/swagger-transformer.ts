...
export class SwaggerTransformer {
  public normalizePaths(
    ...
    const groupedByPath = groupBy(roots, ({ root }: Record<'root', any>) => root.path);
    const paths = mapValues(groupedByPath, routes => {
      const keyByMethod = keyBy(routes, ({ root }: Record<'root', any>) => root.method);
  ...
  public unescapeColonsInPath(app: INestApplication, moduleRoutes: ModuleRoute[]): ModuleRoute[] {
