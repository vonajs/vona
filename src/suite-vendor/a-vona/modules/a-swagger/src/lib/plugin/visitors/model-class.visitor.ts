...
export class ModelClassVisitor extends AbstractFileVisitor {
  ...
  visit(sourceFile: ts.SourceFile, ctx: ts.TransformationContext, program: ts.Program, options: PluginOptions) {
    ...
    const visitClassNode = (node: ts.Node): ts.Node | undefined => {
  ...
  visitPropertyNodeDeclaration(
    ...
    const hidePropertyDecoratorExists = getDecoratorOrUndefinedByNames(
      classTransformerShim ? [ApiHideProperty.name, 'Exclude'] : [ApiHideProperty.name],
      decorators as any,
      factory,
    );
  ...
  addMetadataFactory(
    ...
    const returnValue = factory.createObjectLiteralExpression(
      Object.keys(classMetadata).map(key =>
        factory.createPropertyAssignment(factory.createIdentifier(key), classMetadata[key]),
      ),
    );
  ...
  createDecoratorObjectLiteralExpr(
    factory: ts.NodeFactory,
    node: ts.PropertyDeclaration | ts.PropertySignature | ts.ParameterDeclaration,
    typeChecker: ts.TypeChecker,
  ...
  createEnumPropertyAssignment(
    ...
    if (!isEnum(type) || isEnumMember) {
      if (!isEnumMember) {
        type = isAutoGeneratedEnumUnion(type, typeChecker) as any;
      }
      if (!type) {
        return undefined;
      }
  ...
  createValidationPropertyAssignments(
    ...
    if (!options.readonly) {
      ...
      this.addPropertyByValidationDecorator(factory, 'IsIn', 'enum', decorators as any, assignments, options);
    }
  ...
  addPropertiesByValidationDecorator(
    ...
    const decoratorRef: ts.Decorator | undefined = getDecoratorOrUndefinedByNames([decoratorName], decorators, factory);
    if (!decoratorRef) {
      return;
    }
  ...
  createDescriptionAndTsDocTagPropertyAssignments(
    ...
    const hasExampleOrExamplesKey =
      hasPropertyKey('example', existingProperties) || hasPropertyKey('examples', existingProperties);
