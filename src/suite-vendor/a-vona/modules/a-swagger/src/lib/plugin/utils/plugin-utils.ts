...
export function getTypeReferenceAsString(
  ...
  if (isArray(type)) {
    ...
    if (!typeName) {
      return { typeName: undefined as any };
    }
    return {
      typeName: `${typeName}`,
      isArray: true,
  ...
  try {
    ...
    if (isEnum(type)) {
      return { typeName: undefined as any, arrayDepth };
    }
    if (type.aliasSymbol) {
      return { typeName: 'Object', arrayDepth };
    }
...
export function replaceImportPath(typeReference: string, fileName: string, options: PluginOptions) {
 ...
 * @param typeChecker
 */
export function isAutoGeneratedEnumUnion(type: ts.Type, typeChecker: ts.TypeChecker): ts.Type | undefined {
  if (type.isUnionOrIntersection() && !isEnum(type)) {
    ...
    const isParentSymbolEqual = type.types.every((item, index) => {
      ...
      if (!(item.symbol as any).parent || item.symbol.flags !== ts.SymbolFlags.EnumMember) {
        return false;
      }
      const symbolType = typeChecker.getDeclaredTypeOfSymbol((item.symbol as any).parent);
      if (symbolType === parentType || !parentType) {
        parentType = symbolType as any;
        return true;
      }
      return false;
    });
...
export function isAutoGeneratedTypeUnion(type: ts.Type): boolean {
  if (type.isUnionOrIntersection() && !isEnum(type)) {
    ...
    const undefinedTypeIndex = type.types.findIndex((type: any) => type.intrinsicName === 'undefined');
