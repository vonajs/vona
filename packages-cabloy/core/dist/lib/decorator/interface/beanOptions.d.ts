...
export type TypeDecoratorBeanOptionsSceneBase = 'local' | 'aop' | 'virtual' | 'scope' | 'controller' | 'model' | 'atom' | TypeDecoratorBeanOptionsScene;
export type TypeDecoratorBeanOptionsScene = 'bean' | 'broadcast' | 'startup' | 'queue' | 'version' | 'schedule' | 'middleware' | 'middleware.io' | 'event' | 'stats' | 'sequence' | 'test' | 'local.flow' | 'flow.node' | 'flow.edge' | 'flow.behavior' | 'flow.service' | 'auth.provider' | 'cli.create' | 'cli.default' | 'cli.front' | 'cli.git' | 'cli.store' | 'cli.token' | 'cli.tools' | 'io.message' | 'io.channel' | 'summer.cache' | 'captcha.provider' | 'sms.provider';
