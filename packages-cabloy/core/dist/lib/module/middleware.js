...
function default_1(app) {
    ...
    const ebMiddlewaresNormal = app.meta.middlewaresNormal;
    ...
    // load middlewares
    loadMiddlewares(ebMiddlewaresAll, ebMiddlewaresNormal, ebMiddlewaresGlobal, ebMiddlewaresSocketIoConnection, ebMiddlewaresSocketIoPacket);
    return [ebMiddlewaresNormal, ebMiddlewaresGlobal];
}
exports.default = default_1;
function loadMiddlewares(ebMiddlewaresAll, ebMiddlewaresNormal, ebMiddlewaresGlobal, ebMiddlewaresSocketIoConnection, ebMiddlewaresSocketIoPacket) {
    // load
...
function loadMiddlewaresAll(ebMiddlewaresAll, ebModulesArray, app) {
    for (const module of ebModulesArray) {
        ...
        for (const middlewareKey in config.middlewares) {
            ...
            if (!beanName)
                throw new Error(`bean not set for middleware: ${module.info.relativeName}.${middlewareKey}`);
            let bean;
...
function _swap(middlewares) {
    ...
    middlewaresClone.forEach(item => {
        ...
        if (typeof deps === 'string')
            deps = deps.split(',');
        deps.forEach(dep => {
            if (swapDep(middlewares, dep, item.name))
                result = true;
        });
    });
    return result;
