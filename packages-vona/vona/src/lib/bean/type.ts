...
export type TypeBeanRecordGeneralSelector<SCENE extends string> = {
  [K in keyof IBeanRecordGeneral as K extends `${string}.${SCENE}.${string}` ? K : never]: IBeanRecordGeneral[K];
};
export type TypeBeanRecordGeneralSelectorKeys<SCENE extends string> = keyof TypeBeanRecordGeneralSelector<SCENE>;
