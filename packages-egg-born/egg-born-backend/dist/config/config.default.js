...
const { detectStatus, detectErrorMessage,
/* accepts,*/
 } = require('egg-onerror/lib/utils');
// eslint-disable-next-line
module.exports = appInfo => {
    const config = {};
    const _maintenanceEnus = 'Under development and maintenance, the source code mode may be unstable. Please keep an eye on the development progress, or use the project mode to create a CabloyJS project.';
    const _maintenanceZhcn = '正在开发维护当中，源码模式可能存在不稳定情况。请持续关注开发进度，或者使用项目模式来创建CabloyJS项目。';
    ...
    config.onerror = {
        appErrorFilter(err, ctx) {
            // 422
            if (err && err.code === 422 && !ctx.app.meta.isTest)
                return false;
            // 423
            if (err && err.code === 423 && !ctx.app.meta.isTest)
                return false;
            // 403
            if (err && err.code === 403) {
...
function getFullPath(ctx, dir, filename, _options) {
    ...
    // files that can be accessd should be under options.dir
    if (fullPath.indexOf(module.static.backend) !== 0)
        return null;
    return fullPath;
}
