...
const koa_jwt2_1 = __importDefault(require("koa-jwt2"));
...
function __combineJwtToken(app, options, token, key) {
    ...
    return jsonwebtoken_1.default.sign(payload, options.secret);
}
...
module.exports = (options, app) => {
    ...
    const _koajwt = (0, koa_jwt2_1.default)(options);
    async function _handleNext(ctx, next) {
        ...
        if (useJwt) {
            ...
            if (ctx.state.jwt) {
                ...
                if (isValid) {
                    ...
                    const res = ctx.cookies.keys.decrypt(utility_1.default.base64decode(token, true, 'buffer'));
        ...
        if (useJwt && ctx.response.type === 'application/json' && (isRefreshToken || ctx.response.get('set-cookie'))) {
            ...
            // jwt
            const token = utility_1.default.base64encode(ctx.cookies.keys.encrypt(cookiesRes), true);
