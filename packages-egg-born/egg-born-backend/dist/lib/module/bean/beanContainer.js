...
module.exports = (app, ctx) => {
    const beanContainer = {
            ...
            if (!is.class(beanFullName)) {
                for (const key in app.meta.aops) {
                    ...
                    if (key === beanFullName)
                        continue;
                    // check if match aop
                    if (_beanClass.aop && !aop.matchAop)
                        continue;
                    // match
        ...
        _getAopChainsProp(beanFullName, methodName, methodNameMagic) {
            ...
            const _beanClass = this._getBeanClass(beanFullName);
            if (_beanClass[chainsKey])
                return _beanClass[chainsKey];
...
function __getPropertyDescriptorStatic(obj, prop) {
    ...
    while (proto) {
        ...
        if (descriptor)
            return { descriptor, dynamic: false };
...
function __methodTypeOfDescriptor(descriptorInfo) {
    ...
    if (descriptor.get)
        return null;
    const methodType = (_b = (_a = descriptor.value) === null || _a === void 0 ? void 0 : _a.constructor) === null || _b === void 0 ? void 0 : _b.name;
