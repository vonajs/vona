...
const transports = [ this.bean.logger.makeTransportFile(clientInfo, 'order'), this.bean.logger.makeTransportConsole(clientInfo), ].filter(item => !!item); return { transports }; }, }, };
...
- `makeTransportFile`: Used to construct the file channel, requiring the log file name `order`.

- Since the file name template is `${filename}-%DATE%.log`, the actual generated file name is `order-2025-11-14.log`.

- `makeTransportConsole`: Used to construct the console channel.

- `filter`: The newly created channel may be empty, requiring filtering.

## Obtaining a Logger Client Instance

There are two ways to obtain a Logger instance:
...
// logger: order this.$loggerClient('order').info('test');
...
Due to log hierarchy, some leveled messages are not written to files. Therefore, constructing excessively large message contents will waste system resources.
...
Multi-level Child Loggers can be created from the current Logger instance, thus passing metadata to the log.

``` typescript

this.$logger.child({ requestId: '451' }).child({ extra: 'some info' }).info('test');

this.$logger.child({ requestId: '578', extra: 'some info' }).info('test'); }).info('test');

```

As shown in the image below: `requestId/extra`
