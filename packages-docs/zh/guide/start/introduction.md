...
- `Admin中后台支持SSR`: 对`Admin中后台`应用提供了完整的 SSR 支持，`侧边栏`、`多语言`、`明暗主题`、`品牌色主题`等界面元素均支持 SSR，当再次刷新页面时，显著减少界面跳动，提升用户交互体验
...
许多框架用最简单的案例来证明性能，却忽视了业务复杂性带来的挑战。随着业务增长，项目性能往往会下降，开发者需要投入大量精力进行优化。Vona 从框架核心引入缓存策略，内置`二级缓存`、`Query 缓存`和`Entity 缓存`等机制，让开发者可以专注业务逻辑，无需复杂优化，代码依然保持优雅和直观
