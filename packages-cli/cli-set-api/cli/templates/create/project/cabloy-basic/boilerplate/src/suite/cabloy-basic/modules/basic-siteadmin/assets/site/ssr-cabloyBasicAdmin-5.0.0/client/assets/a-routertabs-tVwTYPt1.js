...
import { B as deepExtend, Y as useComputed, h as BeanScopeBase, k as BeanInfo, l as prepareComponentOptions, u as useController, w as Use, z as deepEqual } from "./zova-BXlOOlVL.js";
	...
	ModelTabs$1 = (_dec$2 = Model({
		...
		deleteTab(tabKey, noActiveNext) {
			...
			return _asyncToGenerator(function* () {
				...
				if (!noActiveNext && index === _this2.tabCurrentIndex) {
					const tabCurrentIndex = index - 1 > -1 ? index - 1 : index + 1 < _this2.tabs.length ? index + 1 : -1;
		...
		deleteTabItem(tabKey, componentKey, noActiveNext) {
			...
			return _asyncToGenerator(function* () {
				...
				const indexItem = tab.items.findIndex((item) => item.componentKey === componentKey);
				if (indexItem === -1) return false;
				if (tab.items.length === 1 && !tab.affix) yield _this3.deleteTab(tabKey, noActiveNext);
				else {
					const items = mutate(tab.items, (copyState) => {
