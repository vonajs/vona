...
import { d as _objectSpread2, f as init_objectSpread2, i as init_objectWithoutProperties, r as _objectWithoutProperties } from "./fecha-5qJk_cbF.js";
import { n as ScopeModuleADate, r as TableCellDate, t as init_src } from "./a-date-C1WkblJu.js";
//#region node_modules/.pnpm/luxon@3.7.2/node_modules/luxon/build/es6/luxon.mjs
...
function getCachedRTF(locString, opts = {}) {
	const { base } = opts, cacheKeyOpts = _objectWithoutProperties(opts, _excluded);
...
function getCachedWeekInfo(locString) {
	...
	if (!data) {
		...
		if (!("minimalDays" in data)) data = _objectSpread2(_objectSpread2({}, fallbackWeekSettings), data);
...
function parseZoneInfo(ts, offsetFormat, locale, timeZone = null) {
	...
	const modified = _objectSpread2({ timeZoneName: offsetFormat }, intlOpts);
...
function combineExtractors(...extractors) {
	return (m) => extractors.reduce(([mergedVals, mergedZone, cursor], ex) => {
		...
		return [
			_objectSpread2(_objectSpread2({}, mergedVals), val),
			zone || mergedZone,
			next
		];
	}, [
...
function clone(inst, alts) {
	...
	return new DateTime(_objectSpread2(_objectSpread2(_objectSpread2({}, current), alts), {}, { old: current }));
}
...
function adjustTime(inst, dur) {
	const oPre = inst.o, year = inst.c.year + Math.trunc(dur.years), month = inst.c.month + Math.trunc(dur.months) + Math.trunc(dur.quarters) * 3, c = _objectSpread2(_objectSpread2({}, inst.c), {}, {
		year,
		month,
...
function parseDataToDateTime(parsed, parsedZone, opts, format, text, specificOffset) {
	...
	if (parsed && Object.keys(parsed).length !== 0 || parsedZone) {
		const interpretationZone = parsedZone || zone, inst = DateTime.fromObject(parsed, _objectSpread2(_objectSpread2({}, opts), {}, {
...
var init_luxon = __esmMin((() => {
	...
	ianaZoneCache = /* @__PURE__ */ new Map();
	...
	intlLFCache = {};
	intlDTCache = /* @__PURE__ */ new Map();
	intlNumCache = /* @__PURE__ */ new Map();
	intlRelCache = /* @__PURE__ */ new Map();
	sysLocaleCache = null;
	intlResolvedOptionsCache = /* @__PURE__ */ new Map();
	weekInfoCache = /* @__PURE__ */ new Map();
	PolyNumberFormatter = class {
		constructor(intl, forceSimple, opts) {
			this.padTo = opts.padTo || 0;
			this.floor = opts.floor || false;
			const { padTo, floor } = opts, otherOpts = _objectWithoutProperties(opts, _excluded2);
			if (!forceSimple || Object.keys(otherOpts).length > 0) {
				const intlOpts = _objectSpread2({ useGrouping: false }, opts);
	...
	PolyDateFormatter = class {
		constructor(dt, intl, opts) {
			...
			const intlOpts = _objectSpread2({}, this.opts);
		...
		formatToParts() {
			...
			if (this.originalZone) return parts.map((part) => {
				if (part.type === "timeZoneName") {
					...
					return _objectSpread2(_objectSpread2({}, part), {}, { value: offsetName });
				} else return part;
			});
			return parts;
		}
	...
	PolyRelFormatter = class {
		constructor(intl, isEnglish, opts) {
			this.opts = _objectSpread2({ style: "long" }, opts);
	...
	Locale = class Locale {
		...
		redefaultToEN(alts = {}) {
			return this.clone(_objectSpread2(_objectSpread2({}, alts), {}, { defaultToEN: true }));
		}
		redefaultToSystem(alts = {}) {
			return this.clone(_objectSpread2(_objectSpread2({}, alts), {}, { defaultToEN: false }));
		}
	...
	digitRegexCache = /* @__PURE__ */ new Map();
	now = () => Date.now(), defaultZone = "system", defaultLocale = null, defaultNumberingSystem = null, defaultOutputCalendar = null, twoDigitCutoffYear = 60, defaultWeekSettings = null;
	Settings = class {
		/**
	...
	Formatter = class Formatter {
		...
		formatWithSystemDefault(dt, opts) {
			...
			return this.systemLoc.dtFormatter(dt, _objectSpread2(_objectSpread2({}, this.opts), opts)).format();
		}
		dtFormatter(dt, opts = {}) {
			return this.loc.dtFormatter(dt, _objectSpread2(_objectSpread2({}, this.opts), opts));
		}
		...
		num(n, p = 0, signDisplay = void 0) {
			...
			const opts = _objectSpread2({}, this.opts);
			if (p > 0) opts.padTo = p;
	...
	}, lowOrderMatrix), daysInYearAccurate = 146097 / 400, daysInMonthAccurate = 146097 / 4800, accurateMatrix = _objectSpread2({
		years: {
			quarters: 4,
			months: 12,
	...
	Duration = class Duration {
		...
		*/
		toFormat(fmt, opts = {}) {
			const fmtOpts = _objectSpread2(_objectSpread2({}, opts), {}, { floor: opts.round !== false && opts.floor !== false });
		...
		*/
		toHuman(opts = {}) {
			...
			const l = orderedUnits$1.map((unit) => {
				...
				return this.loc.numberFormatter(_objectSpread2(_objectSpread2({
					style: "unit",
					unitDisplay: "long"
		...
		*/
		set(values) {
			if (!this.isValid) return this;
			const mixed = _objectSpread2(_objectSpread2({}, this.values), normalizeObject(values, Duration.normalizeUnit));
	...
	Interval = class Interval {
		...
		*/
		count(unit = "milliseconds", opts) {
			...
			let end;
			if (opts === null || opts === void 0 ? void 0 : opts.useLocaleWeeks) end = this.end.reconfigure({ locale: start.locale });
			else end = this.end;
	...
	spaceOrNBSP = `[ ${String.fromCharCode(160)}]`;
	...
	zoneOffsetGuessCache = /* @__PURE__ */ new Map();
	DateTime = class DateTime {
		...
		*/
		set(values) {
			...
			if (containsGregorMD && containsOrdinal) throw new ConflictingSpecificationError("Can't mix ordinal dates with month/day");
			let mixed;
			if (settingWeekStuff) mixed = weekToGregorian(_objectSpread2(_objectSpread2({}, gregorianToWeek(this.c, minDaysInFirstWeek, startOfWeek)), normalized), minDaysInFirstWeek, startOfWeek);
			else if (!isUndefined(normalized.ordinal)) mixed = ordinalToGregorian(_objectSpread2(_objectSpread2({}, gregorianToOrdinal(this.c)), normalized));
			else {
		...
		*/
		toObject(opts = {}) {
			if (!this.isValid) return {};
			const base = _objectSpread2({}, this.c);
		...
		*/
		toRelativeCalendar(options = {}) {
			if (!this.isValid) return null;
			return diffRelative(options.base || DateTime.fromObject({}, { zone: this.zone }), this, _objectSpread2(_objectSpread2({}, options), {}, {
				numeric: "auto",
				units: [
					"years",
					"months",
...
//#endregion
init_src();
export { ScopeModuleADate, TableCellDate, init_luxon as n, DateTime as t };
